...
var cnfgPath = "./config/private.spo-azureclientcreds.json"
...
func TestAuthEdgeCases(t *testing.T) {
	...
	t.Run("ReadConfig/MissedConfig", func(t *testing.T) {
		...
		filePath := u.ResolveCnfgPath("./tmp/private.azureclientcreds.malformed.json")
	...
	t.Run("WriteConfig", func(t *testing.T) {
		folderPath := u.ResolveCnfgPath("./tmp")
		filePath := u.ResolveCnfgPath("./tmp/private.azureclientcreds.json")
		cnfg := &AuthCnfg{
			SiteURL: "test",
		}
		_ = os.MkdirAll(folderPath, os.ModePerm)
